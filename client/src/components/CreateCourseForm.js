...
function CreateCourseForm() {
  ...
  const formSchema = yup.object().shape({
    ...
    picture: yup.mixed().required('Add a picture'),
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      formData.append('instructor_id', values.instructor_id);
      ...
      fetch('/course', {
        ...
        .then((res) => {
          if (res.ok) {
            ...
            throw new Error('Error occurred during course creation');
          }
        })
        .then((post) => {
          setCourse(post);
        ...
        .catch((error) => {
          console.error('Error occurred during course creation:', error);
        });
    },
  });
      ...
      <div className="w-102 bg-blue shadow-lg rounded-lg px-8 py-6">
        ...
        <form onSubmit={formik.handleSubmit} onReset={formik.handleReset} encType="multipart/form-data">
          ...
          <div className="flex justify-center mt-6">
            <button
              type="submit"
              className="px-4 py-2 bg-blue-600 text-white rounded-full w-48 hover:bg-blue-500"
            >
              Submit
            </button>
          </div>
